  ...
  return (
    <div className="space-y-px bg-[#D4B27A]/10">
      {visible.map((u) => (
        <div
          key={u.id}
          className="bg-[#0A0908] p-6 border-l-2 border-[#D4B27A]/30 hover:border-[#D4B27A] transition-colors"
        >
          <div className="flex items-baseline justify-between gap-4 mb-3 flex-wrap">
            <h3 style={serifItalic} className="text-lg text-[#F5E9D1]">{u.title}</h3>
          ...
          <p style={sans} className="text-[#F5E9D1]/70 leading-relaxed whitespace-pre-wrap">{u.content}</p>
